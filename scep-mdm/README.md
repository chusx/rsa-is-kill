...
**PQC migration plan:** None — RFC 8894 (SCEP 2020) is RSA-only; no PQC algorithm defined
...
RFC 8894 (2020) standardizes SCEP. It does not define any PQC algorithm.
There is no SCEP v3 draft with PQC support. The IETF has no active SCEP/PQC work item.

## Why it's stuck

- SCEP protocol has no algorithm negotiation — client and server must agree on RSA.
  The entire PKIMessage format is PKCS#7, which predates PQC
- NDES (the dominant enterprise SCEP server) is a Windows Server component.
  Windows Server has no PQC SCEP support and no announced roadmap
- Apple MDM configuration profiles specify SCEP with `KeySize` (always RSA-2048).
  The MDM profile schema has no PQC key type
