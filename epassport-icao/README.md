...
- Forging a SOD from any country requires only the CSCA public key + a factoring break
- The ICAO PKD has no non-RSA certificate format defined
...
A factoring break attacking passive authentication: factor any CSCA's RSA key → forge
any passport from that country. Border control terminals worldwide would accept
the forgery. A factoring break attacking Active Authentication: clone any passport chip
...
- the Active Authentication public key is on the chip and readable by any NFC reader. a factoring break recovers the AA private key and the chip clone passes active authentication. the document fraud is completely invisible
