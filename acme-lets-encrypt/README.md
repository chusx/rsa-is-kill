...
**PQC migration plan:** None — RFC 8555 (ACME) doesn't define PQC JWS algorithms; Let's Encrypt has no announced PQC account key support; Certbot has no PQC roadmap
...
the 90-day cert rotation people keep pointing to as "quantum-safe" isn't the problem.
...
`acme_rsa_account.py` — `jwk_from_rsa_key()` (JWK with RSA-2048 n/e fields — the CRQC
