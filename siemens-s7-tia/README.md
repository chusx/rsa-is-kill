...
**PQC migration plan:** None — S7comm-plus protocol spec is proprietary and not publicly versioned; Siemens has published zero PQC roadmap for SIMATIC PLCs
...
- S7comm-plus is a closed, proprietary Siemens protocol. There is no public spec,
  no working group, no RFC process. Siemens decides when and if PQC gets added.
...
- Siemens has published no PQC roadmap for SIMATIC. The TIA Portal 2024 release notes
  mention nothing about post-quantum.
