...
- The NSS certificate database format (`cert9.db`, `key4.db`) has no non-RSA key type.
 `certutil -g` (NSS key generation tool) has no non-RSA option
- RHEL system crypto migration to non-RSA requires coordinating NSS, OpenSSL (FIPS module),
 GnuTLS, and Java's NSS bridge — Red Hat has published intentions but no GA date
- Thunderbird S/MIME is blocked by the same LAMPS WG S/MIME non-RSA gap as smime-email/
...
- the freebl RSA operations are what NSS calls for every TLS handshake. the
 ...
 is what the factoring algorithm inverts when it solves the factoring problem
