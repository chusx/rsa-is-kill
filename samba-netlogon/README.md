...
**PQC migration plan:** None — Samba provisioning generates RSA-2048 by default; no PQC KDC cert support; GnuTLS NORMAL priority has no PQC
...
Neither hx509 nor GnuTLS has PQC algorithm support in deployed versions.

## Why it's stuck

- `samba-tool domain provision` has no `--cert-algorithm` flag. RSA-2048 is
  hardcoded as the provisioning certificate algorithm
- Heimdal hx509 (the X.509 library used for PKINIT) has no PQC algorithm OID support
- GnuTLS `NORMAL` priority string (used for Samba TLS) does not include any PQC
