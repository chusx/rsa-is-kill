...
basis for trust verification. Anyone who fetches it has the input an attacker needs for RSA-4096 and
RSA-2048 factoring.
...
- the offline root key story: "keep the root key offline and it's safe." the public key
 ...
 "offline" is irrelevant against the factoring algorithm.
...
`BuildRootMetadata()` (root.json with RSA-4096/RSA-2048 public keys — the input an attacker needs file).
