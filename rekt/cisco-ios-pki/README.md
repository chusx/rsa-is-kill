...
- `crypto key generate` in IOS-XE supports only `rsa` and `ec` key types. There is no
 `ml-dsa` or non-RSA equivalent. Cisco would need a software update for every IOS-XE device.
- Cisco device firmware updates in enterprise environments go through change management,
 testing, and maintenance windows. A network-wide IOS-XE update to support non-RSA keys
 is a multi-year project for large enterprises.
- IKEv2 non-RSA drafts (draft-ietf-ipsecme-ikev2-pqc-*) are still in IETF development.
...
`ip ssh rsa keypair-name` (SSH host key), and the absence of non-RSA key types in `crypto key generate ?`.
