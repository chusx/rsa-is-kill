...
**PQC migration plan:** None — no PQC URI registered in W3C XMLDSig namespace; no active W3C work item for PQC XMLDSig
...
Working Group (it closed in 2013). Without a registered URI, any PQC XMLDSig signature
...
- SAML specifications (SAML 2.0, 2005) reference specific XMLDSig algorithm URIs.
  ...
  would need simultaneous updates to accept new PQC signature algorithm URIs
- SimpleSAMLphp and mod_auth_mellon use xmlsec1 as a library. If xmlsec1 doesn't
  support a PQC algorithm, nothing using xmlsec1 can either
