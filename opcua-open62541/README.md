...
has minimal deployment. No non-RSA policy exists or is planned in the OPC-UA spec.
...
would need to publish a new Part 7 revision with a non-RSA policy URI, and every
vendor would need to implement it. The installed base cannot be updated.

A factoring break targeting industrial infrastructure could:
...
OPC-UA Basic128Rsa15 is still deployed on real industrial control systems. it uses RSA-PKCS1-v1.5, which is classically weak too (Bleichenbacher), and completely dead against a factoring attack.
...
- Basic256Sha256 uses RSA-2048 OAEP, which is better classically, but the factoring algorithm still kills it. session key exchange is RSA-wrapped
